...
## 2026-07-10 · P14 · Publish is a SQL function (the only config write path); the diff also caught SSP day-one

`publish_config_version` (migration 0013, SECURITY DEFINER, service-role only — no authenticated grant) enforces the ≥10-character audit note in the database itself, supersedes the previous live version, promotes the target, and writes the append-only `config.published` event; publishing an already-live version is rejected. The PGlite suite proves all of it offline (note required / statuses flip / event lands / double-publish rejected) — the P14 VERIFY. The admin action wraps it behind `requireAdmin()` and then runs the uprating re-check (J4) immediately, closing the publish→re-check loop from Prompt 9. The pure diff (`lib/rules/config-diff.ts`, golden-tested on 2026.1→2026.2) surfaced that the canonical pair differs by FIVE keys, not four: the four wage bands AND `ssp.day_one` (false→true) — the golden now pins all five. `published_by` remains a profiles FK; admins without a profile row are recorded by email in the event payload instead.
Approved by: build session.

## 2026-07-10 · P14 · Admin access: email allowlist + 404 hard-block; console is flight-recorder-framed

Admin auth is `ADMIN_EMAILS` (comma-separated env; empty list blocks EVERYONE — no default admin). Non-allowlisted users get `notFound()` — the surface doesn't acknowledge its own existence. The predicate is pure and golden-tested (case/space-insensitive, suffix-spoof `…@x.co.uk.evil.com` blocked). Data reads use the service role only behind the guard, and every admin server action re-checks it. The console is one desktop-first client surface with the export's dark a-* theme (CSS extracted and `.adm`-scoped like the marketing port): users & subscriptions with drill-in, the examination log (verdict/defect/search filters, expandable attempt detail with version + checklist-hash chips — read-only by construction: no write action exists for it), the config publisher (diff view + note + typed confirmation), the monitor queue, and feedback. The exam log drawer's full prototype chrome was compacted to expandable rows — same information, admin-internal surface; logged as the one appearance deviation.
Approved by: build session.

## 2026-07-10 · P14 · Monitor queue: manual entry; approval creates a DRAFT clone — nothing auto-publishes

`monitor_findings` (migration 0013, service-role only) holds source changes pending human review, entered manually for now (the Monitor agent is P1 and will write to the same table). Approving a config-kind finding clones the LIVE version into a new DRAFT config version (label suffixed, audit note pointing back at the finding) for the admin to edit and publish separately through the full note+confirm flow — the publish path stays single. Dismissals are recorded with reviewer + timestamp. The feedback widget (FR-8.5) shipped in the same prompt: quiet 1–5 stars + optional comment on the contract-approved and RTW-done completion screens, best-effort submission that can never block a flow, rows insert-only for members (RLS: no member reads; PGlite-tested) and read in the admin console.
Approved by: build session.

## 2026-07-10 · P15 · PWA: shell-only service worker; icons generated programmatically; device installability is a deploy check

The service worker (`public/sw.js`) caches the SHELL only: `_next/static`, fonts, icons, marketing assets (cache-first) and the precached `/offline` fallback served when a navigation fails. Authenticated pages and API data are NEVER `cache.put` — navigations are network-first with the offline page as the only fallback, and non-GET requests are untouched (no background replay: mutations fail loud with preserved input instead, which is honest and simpler than a sync queue). Registration is production-only. The install icons (192/512/maskable) are GENERATED by `scripts/gen-icons.mjs` — a dependency-free PNG writer drawing the ink-900 rounded square with the verified-green seal check — no design-tool export needed and regenerable at any size. The install prompt fires only after a `markModuleCompleted()` event (set on the contract-approved and RTW-done screens), never on first load, waits for the NEXT visit (never mid-flow), respects a 30-day dismissal, detects already-installed standalone mode, and gives iOS Safari add-to-home-screen instructions (no beforeinstallprompt there). Actual add-to-home-screen verification on Android Chrome + iOS Safari, and the Lighthouse run, need real devices/a served deploy — moved to the P16 launch checklist, not claimed here.
Approved by: build session.

## 2026-07-10 · P15 · Save/resume audit: RTW was the gap; photos are deliberately NOT resumed

FR-8.6 audit across every multi-step flow: status advisor (journey_state.status_advisor, P04) ✓, contract questionnaire (journey_state.contract, debounced per-field, P06) ✓, onboarding + gap questionnaire (profiles.onboarding_state, P03) ✓ — and RIGHT TO WORK had zero persistence (all client state). Fixed: `journey_state.rtw` now persists the walkthrough per step (view, worker, route, ticked confirmations, result choice, expiry, checker) with a "Saved just now" indicator in the walkthrough top bars; the page resumes a killed tab exactly where it stopped, and the draft clears on completion or return home. The captured evidence PHOTO is deliberately not persisted server-side per-step: an RTW check must be conducted with the person present, so after an interruption the photo is retaken — resuming a stale photo would weaken the statutory excuse. The pre-auth readiness check gained sessionStorage persistence (server persistence rightly starts at signup). Flaky-network posture: a calm offline banner in the app shell ("everything you've done is saved"), autosave indicators show Saving…/Saved truthfully, and every mutation already fails loud with preserved input.
Approved by: build session (photo-resume exclusion is a compliance judgement, logged).

## 2026-07-10 · P16 · Final gate: grep gates are executable scripts; decorative-green is a reviewed ratchet

`pnpm gate` = typecheck + lint + all 200 tests + three gate scripts, and CI (`.github/workflows/ci.yml`) blocks on any failure, plus a Lighthouse job against the built app. The statutory-literals gate scans app/components/lib/content for the canonical rate literals (allowing only the config seed and test fixtures) AND requires marketing/guidance content to carry no raw £ figures at all (config tokens only). "Decorative green" cannot be judged mechanically, so that gate is a RATCHET: the per-file count of verified-green usages is pinned in a committed allowlist — any new usage fails CI until a human reviews that it marks a legally-true state or primary CTA and updates the allowlist; the review is the control. The secrets gate scans for live key/JWT/DSN patterns. Security pass: per-business token-bucket rate limits on the two Claude callers (in-memory per instance — honest hard ceiling at launch scale; shared store on the checklist), and RTW evidence uploads now pass type-whitelist + size-cap + magic-byte validation plus an AV hook that fails CLOSED once a provider is configured. Accessibility: focus lands on the new question heading after each step in the one-question flows; the axe browser run is a deploy-time checklist item.
Approved by: build session.

## 2026-07-10 · P16 · Launch state is honest: three legal blockers NOT DONE; deploy-time verifications listed

`docs/LAUNCH-CHECKLIST.md` records the truth: solicitor sign-off (LEGAL-REVIEW flags live in the clause library), ICO registration, and the DPIA (pending PRD Open Question 7) are NOT DONE and block launch; Stripe live keys/domain/support inbox are founder tasks. Everything built-but-unverifiable-offline (Stripe test clocks, Lighthouse run, device installability, axe pass, live end-to-end J1, AV provider, cron registration) is listed as deploy-time verification, not claimed as done. NO-PROTOTYPE flags in code (More screen, About page, ComingSoon scaffold) each have a DECISIONS entry; both LEGAL-REVIEW files are on the checklist. The pack's 16 prompts are complete.
Approved by: build session.
...
## 2026-07-12 · Launch ops · Material Symbols subset to used glyphs (3MB → ~160KB)

The full variable icon font (deferred at P01 "to the performance pass") measured as THE mobile LCP killer: 17.1s LCP on throttled mobile, confirmed by a blocked-font A/B (17.1s → 2.5s). Replaced with a ligature-correct subset of the 101 icon names used in the codebase, generated by Google Fonts' icon_names subsetter (all axes kept: opsz/wght/FILL/GRAD). `scripts/subset-icons.mjs` regenerates the subset and writes `public/fonts/icon-names.json`; it MUST be re-run when a new icon name is introduced, or the new icon renders as raw ligature text. Also this session: metric-matched Inter fallback + `font-display: block` on the icon font + preloads (CLS 0.162 → 0.011); Lighthouse CI now audits production with 3 runs (median) instead of a cold localhost server.
Approved by: build session (performance pass promised at P01; no visual change).

## 2026-07-15 · Launch ops · Meta Pixel events wired (founder-directed)
